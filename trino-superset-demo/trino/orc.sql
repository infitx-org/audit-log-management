...
CREATE TABLE IF NOT EXISTS hive.audit_logs.some_env_orc (
  trace_id VARCHAR,
  span_id VARCHAR,
...
WITH (
  external_location = 's3a://audit-logs/some-env-orc',
  format = 'ORC',
  partitioned_by = ARRAY['year', 'month', 'day', 'hour']
);


call hive.system.sync_partition_metadata('audit_logs', 'some_env_orc', 'ADD');
...
SELECT 
  trace_id,
  event_action
FROM hive.audit_logs.some_env_orc
LIMIT 10;
